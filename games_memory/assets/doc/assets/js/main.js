...
/**DEFINE*/
const tbodyId = "tbody";
const firebaseCrud = new FirebaseCrudUser(tbodyId);
...
function getDataUser() {
  showPreload();
  firebaseCrud.getDataUsers().then((result)=>{
    hidePreload();
  });
}
...
function showUser(id) {
  ...
  disableForm();
  showPreload();
  const dataUser = firebaseCrud.getDataUser(id);
  dataUser.then((data) => {
    setDataForm(data);
    hidePreload();
  });
...
function deleteUser(id) {
  ...
  if (confirm(textConfirm) == true) {
    firebaseCrud.setDeleteUser(id).then((data) => {
      getDataUser();
    });
  }
  hidePreload();
...
formUser.addEventListener('submit', (e) => {
  ...
  if (validate) {
    firebaseCrud.setCreateUser(jsonArray).then(hideModal());
  } else {
    firebaseCrud.setUpdateUser(getIdUser, jsonArray).then(hideModal());
  }
});
